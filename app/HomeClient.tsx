...
export default function HomeClient() {
  ...
  const displayName =
    auth?.type === "user" ? auth.name : auth?.type === "guest" ? "אורח" : null;
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-white border-b-2 border-black relative overflow-hidden" dir="ltr">
        <div className="absolute inset-0 pointer-events-none">
          <Image src="https://images.unsplash.com/photo-1580582932707-520aed937b7b?auto=format&fit=crop&w=1600&q=80" alt="" fill className="object-cover opacity-[0.06]" sizes="100vw" priority />
        </div>
        <div className="absolute inset-y-0 left-0 w-52 xl:w-72 hidden lg:block overflow-hidden pointer-events-none">
          <Image src="https://images.unsplash.com/photo-1635070041078-e363dbe005cb?auto=format&fit=crop&w=600&q=80" alt="" fill className="object-cover opacity-[0.18]" sizes="(min-width: 1280px) 288px, 208px" />
          <div className="absolute inset-0 bg-gradient-to-r from-transparent to-white" />
        </div>
        <div className="absolute inset-y-0 right-0 w-52 xl:w-72 hidden lg:block overflow-hidden pointer-events-none">
          <Image src="https://images.unsplash.com/photo-1509228468518-180dd4864904?auto=format&fit=crop&w=600&q=80" alt="" fill className="object-cover opacity-[0.18]" sizes="(min-width: 1280px) 288px, 208px" />
        ...
        <div className="flex items-stretch min-h-[420px] md:min-h-[520px]">
          <div className="hidden md:flex flex-col items-center justify-center border-r border-slate-200 px-6 py-10 shrink-0">
            <p className="text-black font-bold text-sm tracking-[0.2em] uppercase whitespace-nowrap" style={{ writingMode: "vertical-rl", transform: "rotate(180deg)" }}>מתמטיקה · שאלון 581</p>
          ...
          <div className="flex-1 flex flex-col items-center justify-center py-16 md:py-24 px-6 text-center">
            <div className="flex items-center gap-4 mb-10 w-full max-w-xs">
              ...
              <p className="text-black font-semibold text-sm tracking-[0.2em] uppercase whitespace-nowrap">כיתה י״א · 5 יחידות לימוד</p>
              <div className="flex-1 h-px bg-slate-300" />
            </div>
            <h1 className="leading-none tracking-tighter text-center w-full" dir="rtl">
              <span className="block font-black text-black" style={{ fontSize: "clamp(2rem, 10vw, 8rem)" }}>הכנה על ידי</span>
              <span className="block font-black text-black mt-1" style={{ fontSize: "clamp(2.5rem, 17vw, 12rem)" }}>תרגול</span>
            </h1>
            <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 mt-10 w-full max-w-md">
              ...
              <p className="text-black text-sm md:text-base text-center" dir="rtl">כל הנושאים, הנוסחאות והדוגמאות הפתורות במקום אחד</p>
              <div className="hidden sm:block flex-1 h-px bg-slate-300" />
            </div>
            <Link href="/test" className="mt-8 inline-flex items-center gap-2 bg-blue-600 text-white font-bold text-base px-8 py-4 rounded-xl hover:bg-blue-700 transition-colors" dir="rtl">
              התחל מבחן מלא ←
            </Link>
          </div>
          <div className="hidden md:flex flex-col items-center justify-center border-l border-slate-200 px-6 py-10 shrink-0">
            ...
            <p className="text-slate-300 font-bold text-sm tracking-[0.2em] uppercase whitespace-nowrap" style={{ writingMode: "vertical-rl" }}>5 יחידות לימוד</p>
          </div>
        </div>
      </section>
      ...
      <div className="flex flex-col md:flex-row items-center border-t border-slate-200" dir="ltr">
        <div className="md:w-1/2 flex items-center justify-center py-8 px-10">
          <svg viewBox="0 0 500 240" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full max-w-md">
            ...
            <path d="M 20 120 C 57 40, 57 40, 95 120 C 133 200, 133 200, 170 120 C 207 40, 207 40, 245 120 C 283 200, 283 200, 320 120 C 357 40, 357 40, 395 120 C 433 200, 433 200, 470 120" stroke="#0d9488" strokeWidth="2.5" strokeLinecap="round" />
        ...
        <div className="md:w-1/2 relative flex items-center justify-center py-10 px-8 overflow-hidden" dir="rtl">
          <div className="relative text-right leading-none">
            ...
            <p className="text-slate-500 text-sm md:text-base font-normal mt-5 leading-relaxed max-w-xs">כשנתקלים בשאלה קשה — נסו לפתור אותה בשתי דרכים שונות.</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-center border-t border-slate-200" dir="ltr">
        <div className="md:w-1/2 relative flex items-center justify-center py-10 px-8 overflow-hidden" dir="rtl">
          <div className="relative text-right leading-none">
            ...
            <p className="text-slate-500 text-sm md:text-base font-normal mt-5 leading-relaxed max-w-xs">פתרון עשרה תרגילים שווה יותר מקריאת מאה עמודי סיכום.</p>
          </div>
        </div>
        <div className="md:w-1/2 flex items-center justify-center py-8 px-10">
          <svg viewBox="0 0 500 240" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full max-w-md">
      ...
      <section className="max-w-5xl mx-auto px-4 py-14">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {topics.map((topic) => (
            <Link key={topic.id} href={`/topic/${topic.id}`} className="group flex flex-col justify-between bg-black rounded-2xl p-6 min-h-48 hover:bg-slate-800 hover:shadow-xl hover:shadow-black/20 transition-all duration-200 hover:-translate-y-0.5">
              <h3 className="font-bold text-white text-lg leading-snug">{topic.title}</h3>
              <p className="text-white/70 text-sm leading-relaxed">{topic.description}</p>
              <div className="flex items-center justify-between pt-3 border-t border-white/20">
                <span className="text-xs text-white/60">{topic.formulas.length + topic.examples.length} נושאים</span>
                <span className="text-xs font-semibold text-white flex items-center gap-1">קרא עוד <span className="group-hover:-translate-x-0.5 transition-transform inline-block">◀</span></span>
              </div>
            </Link>
          ))}
        </div>
            ...
            <div className="flex items-center gap-4">
              {displayName && (
                <span className="text-white/50 text-xs hidden sm:block">
                  מחובר כ: <span className="text-white/80 font-semibold">{displayName}</span>
                </span>
              )}
